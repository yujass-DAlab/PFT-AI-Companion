"""
=========================================================
Spirometry V3 – AWS-Ready with gTTS Audio Prompts
...
try:
    ...
    print("🟢 7. Plotly imported successfully.")
except ImportError:
    PLOTLY_AVAILABLE = False
    print("⚠️ Plotly not installed. Install with: pip install plotly")

print("🟢 8. All imports successful. Building UI...")

# --------------------------------------------------
# GLOBAL MODE & COACHING LOCK
# --------------------------------------------------
CURRENT_MODE = "practice"
COACHING_IN_PROGRESS = False

# --------------------------------------------------
# TTS (Using gTTS – Works on Linux/Windows/Mac)
# --------------------------------------------------

def generate_coaching_audio(text):
    """Generates an MP3 file from text and returns the file path."""
    try:
        tts = gTTS(text=text, lang="en", slow=False)
        temp_dir = tempfile.gettempdir()
        audio_path = os.path.join(temp_dir, "coaching_audio.mp3")
        tts.save(audio_path)
...
# --------------------------------------------------
# COACHING SEQUENCE (Text only – audio will be one combined file)
# --------------------------------------------------
...
def get_unified_chart():
    if not PLOTLY_AVAILABLE:
        return "⚠️ Plotly is not installed. Please run: pip install plotly"

    if not attempt_history:
        fig = go.Figure()
        fig.update_layout(
            height=180,
            yaxis=dict(range=[0, 100], dtick=20, title="Score (%)"),
            xaxis=dict(
                range=[0.5, 8.5],
    ...
    color_map = {"practice": "#f39c12", "live": "#8e44ad"}
    colors = [color_map.get(m, "#95a5a6") for m in modes]
    ...
    fig = px.bar(
        ...
        category_orders={"Mode": ["practice", "live"]}
    )
...
def get_detailed_summary_text():
    if not attempt_history:
        return "⭐ No attempts yet. Complete at least one blow to see your detailed summary."
    ...
    if weakest_avg == avg_blast:
        ...
        if avg_duration < 80:
            advice = f"Your **Duration** needs to be longer. {tip}"
        else:
            advice = f"Your **Duration** is solid – great lung capacity and muscle hold!"
    else:
        focus = "Consistency"
...
# --------------------------------------------------
# COACHING – with gTTS Audio + Streaming Text
# --------------------------------------------------

def run_coaching(mode):
    global CURRENT_MODE, COACHING_IN_PROGRESS
    if COACHING_IN_PROGRESS:
        return "🔴 Coaching already in progress...", get_coaching_html("Please wait for the current coaching to finish."), None
    COACHING_IN_PROGRESS = True
    try:
        CURRENT_MODE = mode
        status = "🟠 **Practice Mode**" if mode == "practice" else "🟣 **Live Mode**"
        
        # Build the full coaching text
        full_text = f"{status}\n\n### 🎧 Listen to the coach...\n\n"
        combined_tts = ""
        for delay, sentence, tts_text in COACH_SEQUENCE:
            full_text += f"**{sentence}**\n\n"
            combined_tts += tts_text + ". "
            time.sleep(delay)  # Simulate streaming (text updates)
        
        full_text += "\n✅ Coaching finished. Click Record to capture your blow!"
        
        # Generate the MP3 audio for the entire sequence
        audio_path = generate_coaching_audio(combined_tts)
        
        return status, get_coaching_html(full_text), audio_path
    finally:
        COACHING_IN_PROGRESS = False
...
def analyze_recording(audio_filepath):
...
def build_spirometry():
    ...
    <style>
        ...
        .dashboard-group .section-header {
            margin-top: 28px !important;
            margin-bottom: 20px !important;
        }
        .dashboard-group .section-header:first-of-type {
    ...
    with gr.Column():
        gr.Markdown("""
        <div class="main-title">AI Spirometry Companion (V3 )</div>
...
3. 🎤 <span style="color:#e74c3c; font-weight:bold;">✖ Clear Audio</span> to reset.
        ...
        # --- COACHING & RESULTS (WITH AUDIO OUTPUT) ---
        with gr.Accordion("📝 Coaching & Results", open=True):
            # NEW: Audio player for the coaching voice (autoplay)
        ...
        with gr.Column(elem_classes="dashboard-group"):
            ...
            gr.HTML('<h3 style="color:#000000; margin-top:8px; margin-bottom:20px; font-weight:700; font-size:26px;">📊 Instant Effort Score</h3>')
            score_gauge = gr.HTML(value=get_score_gauge_html(0))

            gr.HTML('<h3 style="color:#000000; margin-top:28px; margin-bottom:20px; font-weight:700; font-size:26px;">📈 Unified Attempt Progress</h3>')
            attempt_summary = gr.HTML(value=get_attempt_summary_html())
            attempt_chart = gr.Plot(value=get_unified_chart())

            gr.HTML('<h3 style="color:#000000; margin-top:28px; margin-bottom:20px; font-weight:700; font-size:26px;">📊 Detailed Summary & Trend</h3>')
            with gr.Row():
    ...
    def on_practice():
        status, html_text, audio_path = run_coaching("practice")
        return status, html_text, audio_path

    def on_live():
        status, html_text, audio_path = run_coaching("live")
        return status, html_text, audio_path
    ...
    mic_input.change(
        ...
        outputs=[
            ...
            trend_list_box,
            coach_audio  # Stops the audio on analysis
        ]
    )
...
if __name__ == "__main__":
    ...
    with gr.Blocks(title="PFT AI Companion V3 (AWS Audio Fix)") as demo:
        print("🟢 9. Building UI...")
        build_spirometry()
