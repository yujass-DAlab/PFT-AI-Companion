"""
=========================================================
Spirometry V42 – Plotly Chart + UI Polished
...
def get_unified_chart():
    ...
    - Fixed y-axis 0-100
    - Colored bars (orange for Practice, purple for Live)
    - Passing line at 80%
    - Thinner bars
    """
    ...
    if not attempt_history:
        # Return an empty chart with the same style
        fig = go.Figure()
        fig.update_layout(
            height=180,
    ...
    # Color mapping
    color_map = {"practice": "#f39c12", "live": "#8e44ad"}  # Orange, Purple
...
# --- CARD STYLE (Layered: dashboard #EEF2F6, cards #F8F9FA) ---
CARD_STYLE = """
background:#F8F9FA;
color:#222222;
padding:16px;
...
# --------------------------------------------------
# COACHING – with global lock to prevent repeats
# --------------------------------------------------

def run_coaching(mode):
    global CURRENT_MODE, COACHING_IN_PROGRESS
    if COACHING_IN_PROGRESS:
        return "🔴 Coaching already in progress...", get_coaching_html("Please wait for the current coaching to finish.")
    ...
    try:
        ...
        return status, get_coaching_html(full_text)
    finally:
        COACHING_IN_PROGRESS = False
...
def build_spirometry():
    ...
    <style>
        ...
        /* Dashboard: soft blue-gray #EEF2F6 */
        .dashboard-group {
    ...
    with gr.Column():
        gr.Markdown("""
        <div class="main-title">AI Spirometry Companion (V42 🫁)</div>
...
3. 🎤 <span style="color:#e74c3c; font-weight:bold;">✖ Clear Audio</span> to reset.
        ...
        with gr.Column(elem_classes="dashboard-group"):
            ...
            # --- REPLACED gr.BarPlot with gr.Plot (Plotly) ---
    ...
    def on_practice():
        status, html_text = run_coaching("practice")
        return status, html_text

    def on_live():
        status, html_text = run_coaching("live")
        return status, html_text
...
if __name__ == "__main__":
    ...
    with gr.Blocks(title="PFT AI Companion V42") as demo:
        print("🟢 9. Building UI...")
        build_spirometry()
