"""
=========================================================
Spirometry V37 – Visual Hierarchy + Gauge + Conversational Summary
...
# --------------------------------------------------
# CONVERSATIONAL SUMMARY + GAUGE (NEW)
# --------------------------------------------------

def get_score_gauge(score):
    """Returns a formatted markdown gauge with color emoji and label."""
    if score >= 90:
        emoji = "🟢"
        label = "Excellent"
    elif score >= 70:
...
def get_detailed_summary():
    """
    Generates a CONVERSATIONAL summary with trend and improvement/drop language.
    """
    if not attempt_history:
    ...
    # Stars
    if avg_score >= 95: stars = "⭐⭐⭐⭐⭐"
    elif avg_score >= 90: stars = "⭐⭐⭐⭐☆"
    elif avg_score >= 80: stars = "⭐⭐⭐☆☆"
    elif avg_score >= 70: stars = "⭐⭐☆☆☆"
    else: stars = "⭐☆☆☆☆"

    # --- CONVERSATIONAL ADVICE (based on weakest metric) ---
    ...
    # --- TREND: Improvement / Drop since last attempt ---
    trend_message = ""
    if len(scores) >= 2:
        last = scores[-1]
        prev = scores[-2]
    ...
    # If all metrics are decent, add a positive closing
...
def analyze_recording(audio_filepath, current_mode, status_text):
    ...
    return (
        report,
        gauge_text,                 # <-- REPLACES SLIDER
        gr.update(value=chart_data),
        summary_text,
        counter_text,
        reset_visible,
        gr.update(),
        detailed_summary,           # <-- CONVERSATIONAL
        trend_list,
...
# --------------------------------------------------
# BUILD UI (V37 – Visual Hierarchy + Gauge + Conversational Summary)
# --------------------------------------------------
...
def build_spirometry():
    ...
    <style>
        ...
        /* Red "X" for Clear Audio in instructions & match the Record Box clear button */
        .clear-x {
            color: #e74c3c !important;
            font-weight: bold !important;
        }
        .audio-wrap .clear-button {
            color: #e74c3c !important;
        ...
        /* Buttons unchanged but polished */
    ...
    with gr.Column():
        # --- TITLE ---
        gr.Markdown("""
        <div class="main-title">AI Spirometry Companion (V37 🫁)</div>
...
> **Remember:** You are not expected to perform perfectly on your first attempt. The respiratory therapist is there to coach you throughout the procedure on demand and answer any questions you may have.
        ...
        # --- HOW-TO BOX (with RED X) + RECORD BUTTON ---
        with gr.Row():
...
3. 🎤 <span class="clear-x">**✖ Clear Audio**</span> to reset your recording.
                """)
        ...
        # --- DETAILED SUMMARY & TREND (CONVERSATIONAL) ---
...
if __name__ == "__main__":
    ...
    with gr.Blocks(title="PFT AI Companion V37") as demo:
        print("🟢 9. Building UI...")
        build_spirometry()
